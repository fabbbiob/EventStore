...
namespace EventStore.Core.Tests.Services.RequestManagement.ReadMgr {
	...
	public class when_reading_an_event_committed_on_master_and_on_replicas : specification_with_cluster {
		private CountdownEvent _expectedNumberOfRoleAssignments;

		private string _streamId =
			"when_reading_an_event_committed_on_master_and_on_replicas-" + Guid.NewGuid();
		...
		[Test]
		public void should_be_able_to_read_event_from_all_forward_on_replicas() {
			var replicas = GetReplicas();
			var quorum = (replicas.Count() + 1) / 2 + 1;
		...
		[Test]
		public void should_be_able_to_read_event_from_all_backward_on_replicas() {
			var replicas = GetReplicas();
			var quorum = (replicas.Count() + 1) / 2 + 1;
			var successfulReads = 0;
			foreach (var s in replicas) {
		...
		[Test]
		public void should_be_able_to_read_event_from_stream_forward_on_replicas() {
			var replicas = GetReplicas();
			var quorum = (replicas.Count() + 1) / 2 + 1;
			var successfulReads = 0;
			foreach (var s in replicas) {
		...
		[Test]
		public void should_be_able_to_read_event_from_stream_backward_on_replicas() {
			var replicas = GetReplicas();
			var quorum = (replicas.Count() + 1) / 2 + 1;
			var successfulReads = 0;
			foreach (var s in replicas) {
		...
		[Test]
		public void should_be_able_to_read_event_on_replicas() {
			var replicas = GetReplicas();
			var quorum = (replicas.Count() + 1) / 2 + 1;
			var successfulReads = 0;
			foreach (var s in replicas) {
